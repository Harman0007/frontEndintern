...
useEffect(()=>{
  ...
  const response=await axios.get(`https://backend-internarea-xxtu.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://backend-internarea-xxtu.onrender.com/api/application/${id}`,{action})
